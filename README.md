...
- **下班确认写库（取最晚）**：点「确认已下班」时，把实际确认时间写入对应 log 周文件的 `offwork_actual` 列（v8 起存**完整 datetime** `yyyy-MM-dd HH:mm:ss`，加班到次日也显式记录）；同一天多次确认只保留**最晚**一次（更早的确认跳过并写日志）；跨天时长直接由完整日期算出，无歧义（旧 `HH:mm` 行走 `end < start → 视为次日` 推断兼容）
